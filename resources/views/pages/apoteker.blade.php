        ...
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="card-description">Daftar apoteker yang terdaftar.</p>
            ...
            <div class="modal-body">
                <form id="addDataForm">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="nik" id="nik" placeholder="Contoh: 3201234567890123" required>
                    </div>
                    <div class="form-group">
                        <label for="nama">Nama Lengkap</label>
                        <input type="text" class="form-control" name="nama" id="nama" placeholder="Contoh: Dr. Ahmad Santoso" required>
                    </div>
                    <div class="form-group">
                        <label for="email">Email</label>
                        <input type="email" class="form-control" name="email" id="email" placeholder="Contoh: ahmad@email.com" required>
                    </div>
                    <div class="form-group">
...
<script>
    $(document).ready(function() {
        ...
        const FILTER_API_URL = "{{ route('apoteker.filters') }}";
        ...
        function populateFilters() {
            $.get(FILTER_API_URL, function(response) {
                ...
                if (response.jabatan) {
                    response.jabatan.forEach(jabatan => {
        ...
        // [UBAH] renderTable untuk menampilkan data apoteker
        function renderTable(data) {
            const tableBody = $('#dataTableBody');
            tableBody.empty();
            if (data.length === 0) {
                tableBody.append('<tr><td colspan="6" class="text-center">Data apoteker tidak ditemukan.</td></tr>');
                return;
            }
            data.forEach(item => {
                const rowHtml = `
                    <tr>
                        <td><span class="badge badge-dark">${item.nik}</span></td>
                        <td>${item.nama}</td>
                        <td>${item.email}</td>
                        <td><span class="badge badge-primary">${item.jabatan}</span></td>
                        <td>${item.telepon}</td>
                        <td>
        ...
        // Event Listeners
        $('#searchInput, #filterJabatan').on('keyup change', function() {
            fetchData(1);
        });
